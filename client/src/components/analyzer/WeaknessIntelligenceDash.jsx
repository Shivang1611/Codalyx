      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="p-8 flex flex-col bg-[var(--bg-card)]">
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className="w-10 h-10 rounded-xl bg-[var(--cyan)]/10 border border-[var(--cyan)]/30 flex items-center justify-center shrink-0">
              <BookOpen size={20} className="text-[var(--cyan)]" />
            </div>
            <div>
              <h3 className="font-bold text-[var(--text-primary)]">Conceptual Foundation</h3>
              <p className="text-xs text-[var(--text-muted)] mt-1">Total Duration: 7 weeks</p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6 relative pl-4">
            ...
            {currentData.plan.map((step, idx) => (
              <div key={idx} className="relative z-10 flex gap-6">
                <div className="w-6 h-6 rounded-full border-2 border-[var(--cyan)] bg-[var(--bg-card)] text-[var(--cyan)] flex items-center justify-center text-xs font-bold shrink-0">
                  {idx + 1}
                </div>
                <div className="flex-1 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-5 relative overflow-hidden">
                  <div className="absolute right-0 top-0 bottom-0 w-1 bg-[var(--cyan)]" />
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-bold text-[var(--text-primary)]">{step.title}</h4>
                    <span className="text-[10px] font-bold text-[var(--cyan)] bg-[var(--cyan)]/10 px-2 py-1 rounded">{step.duration}</span>
                  </div>
                  <p className="text-xs text-[var(--text-secondary)] mb-4">{step.desc}</p>
                  <ul className="space-y-2">
                    {step.tasks.map((task, tIdx) => (
                      <li key={tIdx} className="flex items-center gap-2 text-xs text-[var(--text-secondary)]">
          ...
          <div className="mt-8 pt-6 border-t border-[var(--border-subtle)]">
            <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase mb-3">Next Steps After Completion</p>
            <div className="flex flex-wrap gap-3">
              <button className="px-4 py-2 border border-[var(--cyan)]/50 text-[var(--cyan)] hover:bg-[var(--cyan)]/10 rounded-lg text-xs font-bold transition-colors">
                Move to problem practice
              </button>
              <button className="px-4 py-2 border border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-[var(--border)] rounded-lg text-xs font-bold transition-colors">
                Join study group
              </button>
              <button className="px-4 py-2 border border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-[var(--border)] rounded-lg text-xs font-bold transition-colors">
